...
RUN apk add --no-cache build-base git python3 npm curl
...
#RUN NODE_ENV=development npm --prefix ./assets ci --progress=false --no-audit --loglevel=error --production=false
...
COPY --from=build --chown="${USER}":"${USER}" /app/_build/"${MIX_ENV}"/rel/sudoku ./

ENTRYPOINT ["bin/sudoku"]

# Usage:
#  * build: sudo docker image build -t elixir/sudoku .
